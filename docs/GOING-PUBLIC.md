...
- Require status checks to pass: select **`check`**, **`cargo-deny`**, and both
  **`windows-smoke (windows-latest)`** and **`windows-smoke (windows-11-arm)`**
  (the CI jobs from `ci.yml` — the Windows smoke-test is a matrix over x64 and
  arm64). Don't add the Release workflow here — it's tag-triggered, not a PR
  check, so it never reports a status on a PR.
